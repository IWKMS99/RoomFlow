...
public interface BookingApiContract {
    ...
    @Operation(
            ...
            description =
                    "Создает бронирование переговорной комнаты для текущего пользователя. Время начала и конца должны быть в будущем.")
    @RequestBody(
    ...
    ResponseEntity<BookingResponseDto> createBooking(CreateBookingRequestDto request);
    ...
    @ApiResponses({
        @ApiResponse(
                responseCode = "200",
                description = "Почасовая сетка занятости всех комнат",
                content =
                        @Content(
                                mediaType = "application/json",
